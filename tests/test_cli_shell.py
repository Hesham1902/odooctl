...
def test_shell_auto_picks_single_db(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli.compose, "run", lambda path, *a, **kw: (calls.append((path, a)), FakeProc())[1])
...
def test_shell_explicit_db_skips_lookup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli.compose, "web_running", lambda p, e: True)
    lookups = []
    monkeypatch.setattr(cli.compose, "databases", lambda p, u="odoo": lookups.append(p) or [])
    ...
    monkeypatch.setattr(cli.compose, "run", lambda path, *a, **kw: (calls.append(a), FakeProc())[1])
